...
if ( ! function_exists( 'woopt_update_checker' ) ) {
	add_action( 'init', 'woopt_update_checker', 99 );
	add_action( 'admin_notices', 'woopt_update_notice', 99 );

	function woopt_update_checker() {
		if ( $key = wpc_get_update_key( 7452 ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . $key . '.json', WOOPT_FILE, plugin_basename( WOOPT_DIR ) );
		}
	}

	function woopt_update_notice() {
		if ( apply_filters( 'wpc_dismiss_notices', false ) ) {
			return;
		}

		if ( get_option( 'wpc_dismiss_notice_woopt_update' ) ) {
			return;
		}
	...
	class WPCleverPremium {
		...
		function admin_enqueue_scripts( $hook ) {
			if ( strpos( $hook, 'wpclever-keys' ) ) {
				wp_enqueue_style( 'wpc-premium', WOOPT_URI . 'includes/premium/css/premium.css' );
				wp_enqueue_script( 'wpc-premium', WOOPT_URI . 'includes/premium/js/premium.js', [ 'jquery', ] );
			}
		}
