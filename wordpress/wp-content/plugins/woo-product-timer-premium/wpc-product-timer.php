		...
		if ( ! class_exists( 'WPCleverWoopt' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoopt {
				...
				function __construct() {
					...
					// AJAX
					add_action( 'wp_ajax_woopt_add_action', [ $this, 'add_action' ] );
				...
				function add_action() {
					...
					$action    = [];
					$global    = isset( $_POST['global'] ) && ( $_POST['global'] === 'true' );
					...
					if ( ! empty( $form_data ) ) {
						...
						if ( isset( $form_action['woopt_actions'] ) && is_array( $form_action['woopt_actions'] ) ) {
							$action = reset( $form_action['woopt_actions'] );
						}
					}

					self::action( 0, $action, $global, true );
