...
// @ts-ignore
const module: HTMLElement = document.querySelector(
...
export default function coupon_sort_events() {
  $(module).on(
    "change",
    "select#_acfw_coupon_sort_select",
    toggleCustomSortValueField
  );
  $(module).on(
    "change",
    "input#_acfw_coupon_sort_priority",
    validateCustomSortValue
  );
...
function toggleCustomSortValueField() {
  ...
  const $customField: JQuery = $(module).find(
    "input#_acfw_coupon_sort_priority"
  );
  // @ts-ignore
