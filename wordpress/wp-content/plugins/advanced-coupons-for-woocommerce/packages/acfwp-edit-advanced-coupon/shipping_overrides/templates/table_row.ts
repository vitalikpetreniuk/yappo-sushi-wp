...
const $: any = jQuery;
const moduleBlock: HTMLElement | null = document.querySelector(
...
export default function table_row_template(object: any) {
  const { shipping_zone, discount_type, discount_value, shipping_method } =
    object;

  const $table: JQuery = $(moduleBlock).find(".shipping-overrides-table");
  const zoneMethods: any = $table.data("zonemethods") || [];
  const zoneData: any = zoneMethods.filter(
    (zm: any) => zm.zone_id == shipping_zone
  );

  if (!zoneData.length) return "";

  const { zone_name, methods } = zoneData[0];
  const selectedMethod: any = methods.filter(
    (m: any) => m.value == shipping_method
  );
  ...
  return `
    <tr>
      <td class="shipping-zone object" data-object="${esc_attr(
