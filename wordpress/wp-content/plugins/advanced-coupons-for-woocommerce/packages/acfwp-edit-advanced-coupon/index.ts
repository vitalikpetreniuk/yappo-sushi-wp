...
jQuery(document).ready(($: any) => {
  if (modules.indexOf('acfw_cart_conditions_module') > -1) initialize_cart_conditions();

  if (modules.indexOf('acfw_add_free_products_module') > -1) add_products_module_events();

  if (modules.indexOf('acfw_auto_apply_module') > -1) auto_apply_coupon_module_events();

  if (modules.indexOf('acfw_shipping_overrides_module') > -1) shipping_overrides_events();
  ...
  if (modules.indexOf('acfw_virtual_coupons_module') > -1) virtual_coupon_events();
