...
class Cart_Conditions extends Base_Model implements Model_Interface, Initiable_Interface {
    ...
    private function _get_has_ordered_product_categories_before_condition_field_value( $condition_data ) {
        ...
        if ( 'within-a-period' === $type['condition'] ) {
            $order_ids = $this->_get_orders_within_a_period( $user_email, array( 'wc-completed' ), $type['value'] );
        } elseif ( 'number-of-orders' === $type['condition'] ) {
            $order_ids = $this->_get_user_previous_orders( $user_email, array( 'wc-completed' ), $type['value'] );
        }
    ...
    private function _get_total_customer_spend_on_product_category_condition_field_value( $condition_data ) {
        ...
        // if billing email is not filled, then fallback to email user meta.
        $user_email = get_user_meta( $current_user->ID, 'billing_email', true );
        $user_email = $user_email ?? $current_user->user_email;
    ...
    private function _get_orders_within_a_period( $email, $statuses = array( 'wc-completed' ), $period = 3, $sort = 'DESC' ) {
        ...
        foreach ( $orders as $order ) {
            ...
            if ( $order->get_status() === 'completed' ) {
                ...
                if ( strtotime( $completed_date ) >= strtotime( $old_date ) && strtotime( $completed_date ) <= strtotime( $latest_date ) ) {
            ...
            if ( $order->get_status() === 'processing' ) {
                ...
                $paid_date = $paid_date->date( 'Y-m-d H:i:s' );
                if ( strtotime( $paid_date ) >= strtotime( $old_date ) && strtotime( $paid_date ) <= strtotime( $latest_date ) ) {
                    $results[] = $order->get_id();
                    continue;
                }
            }
            ...
            if ( in_array( $order->get_status(), array( 'processing', 'completed' ), true ) ) {
                ...
                if ( strtotime( $date ) >= strtotime( $old_date ) && strtotime( $date ) <= strtotime( $latest_date ) ) {
                    $results[] = $order->get_id();
                    continue;
                }
            }
