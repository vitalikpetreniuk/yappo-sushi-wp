...
class Cashback_Coupon extends Base_Model implements Model_Interface {
    ...
    public function remove_cashback_coupon_store_credit_entry( $order, $data_store ) {
        $is_remove_coupon = isset( $_POST['action'] ) && 'woocommerce_remove_order_coupon' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) ?? false; // phpcs:ignore
        $coupon_code = isset( $_POST['action'] ) ? sanitize_text_field( wp_unslash( $_POST['coupon'] ) ) : ''; // phpcs:ignore
