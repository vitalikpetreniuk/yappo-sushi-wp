...
class Script_Loader extends Base_Model implements Model_Interface {
    ...
    public function register_backend_styles( $styles ) {
        $styles['acfwp-edit-advanced-coupon'] = array(
            'src'   => $this->_constants->DIST_ROOT_URL . 'acfwp-edit-advanced-coupon.css',
        ...
        $styles['acfwp-edit-coupon-app'] = array(
            'src'   => $this->_constants->DIST_ROOT_URL . 'acfwp-edit-coupon-app.css',
            'deps'  => array(),
            'ver'   => $this->_constants->VERSION,
            'media' => 'all',
        );
    ...
    public function register_backend_scripts( $scripts ) {
        $scripts['acfwp-edit-advanced-coupon'] = array(
            'src'    => $this->_constants->DIST_ROOT_URL . 'acfwp-edit-advanced-coupon.js',
        ...
        $scripts['acfwp-edit-coupon-app'] = array(
            'src'    => $this->_constants->DIST_ROOT_URL . 'acfwp-edit-coupon-app.js',
            'deps'   => array(),
            'ver'    => $this->_constants->VERSION,
            'footer' => true,
        );
    ...
    public function load_backend_scripts( $screen, $post_type ) {
        ...
        if ( 'post' === $screen->base && 'shop_coupon' === $screen->id && 'shop_coupon' === $post_type ) {
            ...
            wp_enqueue_style( 'acfwp-edit-coupon-app' );
            wp_enqueue_script( 'acfwp-edit-coupon-app' );
        }

        $tab = $_GET['tab'] ?? ''; // phpcs:ignore
        if ( 'toplevel_page_advanced-coupons-network' === $screen->base && in_array( $tab, array( 'acfwp_license', '' ), true ) ) {

            wp_enqueue_style( 'acfw_slmw_css', $this->_constants->DIST_ROOT_URL . 'acfwp-slmw-license.css', array( 'vex', 'vex-theme-plain' ), 'all' );
            wp_enqueue_script( 'acfw_slmw_js', $this->_constants->DIST_ROOT_URL . 'acfwp-slmw-license.js', array( 'vex' ), $this->_constants->VERSION, true );
    ...
    public function load_frontend_scripts() {
        ...
        if ( is_cart() || is_checkout() ) {
            wp_enqueue_script( 'acfwp-cart', $this->_constants->DIST_ROOT_URL . 'acfwp-cart.js', array( 'jquery', 'wc-cart' ), $this->_constants->VERSION, true );
        }
    }
    ...
    public function enqueue_admin_app_scripts() {
        wp_enqueue_script( 'acfwp_admin_app', $this->_constants->DIST_ROOT_URL . 'acfwp-admin-app.js', array(), $this->_constants->VERSION, true );
    }
    ...
    public function enqueue_admin_app_styles() {
        wp_enqueue_style( 'acfwp_admin_app', $this->_constants->DIST_ROOT_URL . 'acfwp-admin-app.css', array(), $this->_constants->VERSION, 'all' );
    }
