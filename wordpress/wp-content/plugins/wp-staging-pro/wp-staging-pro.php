...
if (version_compare(phpversion(), '5.6.0', '>=')) {
    ...
    if (!function_exists('wpstg_unsupported_php_version')) {
        function wpstg_unsupported_php_version()
        {
            ...
            echo '<p>' . sprintf(esc_html__('WP STAGING requires PHP %s or higher. Your site is running an outdated version of PHP (%s), which requires an update.', 'wp-staging'), '5.6', esc_html(phpversion())) . '</p>';
            echo '</div>';
        }
    }
