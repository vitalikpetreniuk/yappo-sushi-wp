(function () {
  ...
  (function ($) {
    window.addEventListener('backups-tab', function () {
      window.history.pushState(null, null, window.location.pathname + '?page=wpstg_backup');
      ...
      if (adminMenuElements.length > 0) {
        for (var i = 0; i < adminMenuElements.length; i++) {
          ...
          if (adminMenuElements[i].querySelector('a') !== null) {
            if (adminMenuElements[i].querySelector('a').getAttribute('href') === 'admin.php?page=wpstg_backup') {
  ...
  (function ($) {
    ...
    WPStagingBackupManageSchedules = {
      listen: function listen() {
        ...
        $('body').on('click', '#wpstg--edit-schedule', WPStagingBackupManageSchedules.updateSchedule);
      },
