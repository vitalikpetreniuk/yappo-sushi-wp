...
class Upsell implements Model_Interface, Initializable_Interface {
    ...
    public function enqueue_upgrade_settings_scripts( $screen, $post_type ) {
        ...
        if ( 'woocommerce_page_wc-admin' === $screen->id || 'edit-shop_coupon' === $screen->id ) {
            wp_enqueue_style( 'acfw-wc-admin', $this->_constants->DIST_ROOT_URL() . 'acfwf-wc-admin.css', array(), Plugin_Constants::VERSION, 'all' );
            wp_enqueue_script( 'acfw-wc-admin', $this->_constants->DIST_ROOT_URL() . 'acfwf-wc-admin.js', array( 'wc-components' ), Plugin_Constants::VERSION, true );
            wp_localize_script(
                'acfw-wc-admin',
                'acfwWCAdmin',
                array(
