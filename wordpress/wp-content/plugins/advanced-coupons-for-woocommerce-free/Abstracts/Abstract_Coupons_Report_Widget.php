    ...
    protected function _query_coupons_table_data() {
        ...
        foreach ( $orders as $order ) {
            ...
            $order_coupons_data = array_map(
                function( $i ) use ( $order, $currency_settings ) {
                    return array(
                        'order_item_id'  => $i->get_id(),
                        'ID'             => $i->get_id(),
                        'code'           => $i->get_code(),
                        'discount'       => apply_filters( 'acfw_filter_amount', $i->get_discount(), $currency_settings ),
                        'discount_tax'   => apply_filters( 'acfw_filter_amount', $i->get_discount_tax(), $currency_settings ),
                        'order_currency' => $order->get_currency(),
                        'extra_discount' => \ACFWF()->Helper_Functions->get_coupon_order_item_extra_discounts( $i, false ),
                    );
                },
                $order->get_coupons(),
            );
