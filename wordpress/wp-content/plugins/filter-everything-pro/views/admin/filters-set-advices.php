...
<div class="wpc-filter-set-advices">
    <ul class="wpc-advices">
        ...
        <li>
            ...
            <div class="wpc-advice-body">
                <?php if ( defined( 'FLRT_FILTERS_PRO' ) && FLRT_FILTERS_PRO ) : ?>
                    <p>In most cases, the most convenient option will be to use the Pop-up widget on mobile. Just enable this option in <em>Filters -> Settings -> Enable Pop-up Filters widget</em>. And on mobile devices will appear the button that opens the Pop-up Filters widget.</p>
                <?php else : ?>
                    <p>In the current Free version of the plugin you can enable the option <strong>Collapse widget and show the Filters opening button</strong> in <em>Filters -> Settings</em>.<br />The <a href="<?php echo FLRT_PLUGIN_URL; ?>" target="_blank" class="wpc-external-link">PRO version</a> allows you to use a more convenient <strong>Pop-up Filters widget</strong> on mobile.<br />After activating any of these options on mobile devices will appear the button that opens Filters widget.</p>
                <?php endif; ?>
