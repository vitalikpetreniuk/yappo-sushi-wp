...
<?php
    ...
    public function filterSetDefaultFields( $defaultFields, $filterSet )
    {
        ...
        $defaultFields['wp_filter_query'] = array(
            ...
            'tooltip'       => wp_kses ( __( 'Every Posts list like "Popular products" or "Recent posts" on a page is related with some WP_Query. This field allows you to set desired Posts list by choosing its WP_Query.<br /><br />If the filtering process does not change Posts you need, it means you selected the wrong WP_Query. Please, try to experiment with different ones, until it starts to filter.', 'filter-everything' )
                ,
                array(
                    'br' => array()
                )
        ...
        foreach ( $defaultFields as $key => $attributes ){
            ...
            if( $key === 'reset_button_text' ){
                $new_fields['apply_button_page_type'] = array(
                    ...
                    'instructions'  => esc_html__('Allows you to display Filters widget on a page different from the page with filtered Posts list', 'filter-everything'),
