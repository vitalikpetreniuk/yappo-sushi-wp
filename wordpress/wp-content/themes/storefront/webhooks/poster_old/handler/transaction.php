...
class Transaction
{
    const LOG_FILE_URL = '/wp-content/uploads/log/poster_webhook_handler.log';
    ...
    private function verifyCheckClose()
    {
        $postData = $this->postData;
        if ($postData['object'] != 'transaction') {
            $this->addLog('verifyCheckClose: Валідація не пройшла. Це не транзакція', $postData['object']);
            throw new \Exception('Валідація не пройшла. Це не транзакція');
        }
        if ($postData['action'] != 'closed') {
            $this->addLog('verifyCheckClose: Валідація не пройшла. Це не закритий чек', $postData['object']);
            throw new \Exception('Валідація не пройшла. Це не закритий чек');
        }
    }
    ...
    public function getCheck(): array
    {
        ...
        $transaction_id = $this->checkId;
        $transactionData = [];
        $per_page = 500;
        $transaction_url = self::POSTER_URL . 'transactions.getTransactions';
        $transaction_url .= '?token=' . self::POSTER_API_TOKEN
            . '&date_from=' . date('Y-m-d', $this->postData['time'])
            . '&date_to=' . date('Y-m-d', $this->postData['time'])
            . '&per_page=' . $per_page
            . '&page=1';

        $transaction_response = wp_remote_get($transaction_url);
        if (is_wp_error($transaction_response)) {
            $this->addLog('getCheck: Не вдалий запит про отримання даних про чек', $transaction_url);
            throw new \Exception('Не вдалий запит про отримання даних про чек');
        }
        $response_ts = wp_remote_retrieve_body($transaction_response);
        $transactions = json_decode($response_ts, true);
        if (!empty($transactions['response']['data'])) {
            foreach ($transactions['response']['data'] as $datum) {
                if ($datum['transaction_id'] == $transaction_id) {
                    $transactionData = $datum;
                    break;
                }
            }
        } else {
            $this->addLog('Request getCheck: transactions.getTransactions data is empty ', $transaction_url);
            $this->addLog('Response getCheck: transactions.getTransactions data is empty ', $transactions);
        }
    ...
    private function prepareDataToPipe()
    {
        ...
        foreach ($checkData['products'] as $product) {
            $products[] = [
                ...
                'num' => $product['num'], //Кільість товару в чекові
            ];
        }
        $prepareDate['client'] = [
            'pipedrive_client_id'=>'',
        ...
        $prepareDate['check'] = [
            ...
            'date_close' => $checkData['date_close'],
            'products' => $products
        ];
        if (!empty($clientData)) {
            $prepareDate['client'] = [
                ...
                'address'=>end($clientData['addresses'])['address1']?end($clientData['addresses'])['address1']:''
            ];
    ...
    private function checkContactInPipe()
    {
        ...
        if (!empty($phone)) {
            ...
            if (empty($contactResponse['data']) || empty($contactResponse['data']['items'][0])) {
                ...
                $this->preapreData['client']['pipedrive_client_id'] = $itemPerson['item']['id'];
...
//            "user_id" => '' // Користувач під яким буде створюватись угода
            ...
            '94ebd4ecea470e6c08206f1a4a16fa3c8e26c3a5' => "У магазині", // Спосіб отримання old 6658944630026abfc0b89d6fe33881a7ef148b0a
...
//            '5d235e313b0ce48b9f65bf088067d92171707346' => $billing_city, // Місто / Село
        ...
        $this->addLog('checkContactInPipe: New Deal ' . $dealId, $dealId);
    ...
    {
        if (empty($data)) {
            file_put_contents($_SERVER['DOCUMENT_ROOT'] . self::LOG_FILE_URL, $title . "\n", FILE_APPEND);
        } else {
            file_put_contents($_SERVER['DOCUMENT_ROOT'] . self::LOG_FILE_URL, $title . "\n" . print_r($data, 1) . "\n", FILE_APPEND);
        }
